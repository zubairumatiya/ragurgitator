// Appraise → Config metrics: the pinned cross-config view (a peer of the config
// tabs, not a config itself; see docs/multi-config-plan.md §6/§8 Phase 4). Reads
// every config's latest frozen eval-run snapshot (appraiseStore) and shows them
// side by side, grouped by corpus so same-corpus A/Bs sit together, with the
// best value per metric highlighted within each group. Standalone (outside
// /c/[configId]), so no per-config banner/sub-nav. Dynamic — it reads the DB per
// request.
...
const ABOUT =
  ...
  "metric is highlighted within each corpus.\n\n" +
