...
//
// Config-scoped ANYWAY, for one value: the PRECISION TARGET the sweep holds
// itself to is a per-config setting (batch_savings.semanticCache.acceptTarget).
// The judged events are per-space and shared; only the target is the scoped
// config's. Note /appraise/semantic-cache is not under /c/<configId>, so
