...
// a retrieval change — and freezes a run snapshot. Incremental and cheap: it
// GENERATES NOTHING (that is Bulk actions → Add, the only path that can also
// route generation through the batch API). Backs the "Score pending" button on
// /eval. Streams progress as NDJSON (one EvalEvent per line) so the dashboard
// can show a live bar + per-question results.
//
