...
// signed-in user has — the bulk sibling of PATCH /api/batch/[id] { action:
// "ack" }, fired when the status panel is opened. Opening the panel is the user
// seeing the jobs, so the dot clearing is the honest reading; the per-row
// Dismiss buttons stay for the rows themselves.
//
