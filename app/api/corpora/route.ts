...
//
// GET lists corpora with document counts for the config-creation corpus picker, the
// sidebar, and the corpora page — including how many docs have stored raw text
// (`embeddableCount`), so the picker can warn when spawning a config over a corpus
// whose docs predate raw-text storage (0010) would yield an empty config.
// `?includeEmpty=1` also returns doc-less corpora.
//
// POST creates a new named corpus — empty, or seeded from existing corpora via
// `fromCorpusIds` (their documents merged and de-duplicated by content hash; `dupes`
// reports what was collapsed).
//
