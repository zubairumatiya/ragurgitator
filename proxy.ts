// PROXY — Next 16's renamed Middleware. Root-level, one per project.
//
...
//      This has to happen somewhere that can write cookies to the RESPONSE, which
//      Server Components cannot do — hence here.
//   2. OPTIMISTIC redirect of cookie-less visitors to /login.
//
// It is NOT an authorization boundary. The Next auth guide is explicit that proxy
// runs on every route including prefetches, so it must stay cheap and must never be
// the only check. Real authorization is lib/auth/dal.ts, called next to the data.
// If this file were deleted, the app should still be secure — only less pleasant
// to use.
