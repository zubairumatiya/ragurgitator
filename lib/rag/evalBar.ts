...
// positive recall min-rate means "must be a hit"; MRR uses the per-question
// reciprocal rank at mrr_k; nDCG is graded). Pure — no
...
export function failsBar(q: BarQuestion, criteria: EvalCriteria): boolean {
  ...
  const { recall, mrr, ndcg } = criteria;
  if (recall.enabled && recall.minRate !== null && recall.minRate > 0 && q.hit === false) {
    return true;
  }
  if (mrr.enabled && mrr.minRate !== null && mrr.minRate > 0 && q.rr !== null && q.rr < mrr.minRate) {
    return true;
  }
