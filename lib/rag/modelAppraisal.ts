// DB + registry layer for Appraise → Models.
//
//   1. listModelRateCard()  — pure registry + price table + env availability. No
//      DB, always fully populated, which is why the Models tab is useful on first
//      open even with zero eval data.
//   2. meteredEmbedTokens() — the one number behind the rate card's free-tier note.
//
// The per-model quality comparison lives in lib/rag/replayStore.
//
// EMBEDDING MODELS ONLY. The answer/judge models are priced in pricing.ts too, but
// nothing measures their quality, so they'd be a table of prices with every metric
// dashed — they belong on the Costs tab.
...
// The LLM rate card — the generation side of the same question. LLM_PRICES was
// priced in the ledger but never SHOWN: you could see what a model had cost you
// after the fact, but not what it would cost before you picked it.
//
// Deliberately NOT merged with RateCardRow. An embedding model is priced on one
// axis (per input token) and an LLM on two, at very different rates; and it has no
// dimension while an LLM has a context window. A union row would be half-null in
// both directions.
...
// OUTPUT RATE. Output is the column that ranks the ladder honestly: at this app's
// prompt sizes generation spend is dominated by the output rate, so sorting on
// input would put the cheap-to-prompt/expensive-to-answer models at the top and
// misrepresent which is actually cheap to run here.
//
...
// Performance: moved. This module used to compute a per-model comparison from
// eval_runs and eval_model_trials. The trial half could not work: a trial re-ranks
// inside a candidate pool that contains the correct chunk by construction, so every
// model scored 1.000 and nothing was comparable. It was replaced by the offline
// replay (0043), which ranks the FULL corpus for each model from vectors already in
// embedding_cache — same questions, same scan, only the model changes.
//
// The full-corpus eval_runs snapshots still power /appraise/configs; the Models tab
// reads them only for the live-vs-replay note under each table.

// --- 3. the free-tier line --------------------------------------------------

// Embed tokens this app has METERED (spend_totals, surface='embed'), for the rate
// card's free-tier note.
//
// Deliberately narrow, and the UI copy must match: this is what THIS APP has
// recorded since cost accounting shipped (0034). It is not per-model, and it is not
// the provider's free-tier counter — Voyage's allowance is per ACCOUNT and covers
// every other use of that key, which we can't see. Never render this as a "% of
// free tier used" bar; the denominators don't line up.
