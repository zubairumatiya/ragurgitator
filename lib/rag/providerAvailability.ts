// PROVIDER AVAILABILITY — "can this user embed/generate with provider X right now?"
//
...
//   local — a HOST CAPABILITY question. transformers.js downloads multi-hundred-MB
//     weights and cannot run on a serverless function, so it stays gated on
//     process.env.LOCAL_EMBEDDINGS. That is not a credential and must not become
//     one: no user can fix it from the UI.
//
// The reason strings differ accordingly, which is the whole point of keeping the
// two paths apart — telling a user to "add a local key in Settings" would send them
