...
//
// "Batch the embedding leg" is a per-config savings setting, and every path about
// to buy ingest embeddings has to honour it or the setting is a lie in whichever
// path forgot. There are two such paths and they used to disagree:
// spawn-from-corpora checked it, a plain file upload never did — so a user with the
// lever on watched uploads embed inline at full price.
//
// ADDITIVE, never fatal. build() returns null for a non-Voyage base model, for
// nothing to embed, and for a document set the embedding cache already covers — in
// each case this returns false and the caller embeds inline.
//
...
// /api/batch/poll applies the results. The caller must NOT then embed inline —
// that would buy the same vectors twice.
//
// Must run inside the target config's withConfig scope.
