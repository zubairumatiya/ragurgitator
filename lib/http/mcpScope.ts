...
//
// configScope.ts opens a request scope from a session COOKIE; this file opens the
// same scope from a verified bearer TOKEN, and that is the entire difference. Both
// do the identical three things in the identical order:
//
...
// pending.run() invokes fn. That is what guarantees the async context Next binds
// for the flush carries no open transaction handle. Flip after() inside withUser
// and the deferred writes inherit a transaction that has already committed by the
// time they run, which fails silently rather than loudly. See the
// AsyncResource.bind note in lib/http/ndjson.ts for the same trap when streaming.
//
...
// RLS reads set_config('app.user_id') set by withUser, the store layer's
// predicates read activeUserId() out of AsyncLocalStorage, and BYOK key resolution
// takes an explicit userId. All three are downstream of a RequestUser and
// indifferent to where it came from. That property is worth protecting: if some
// future code path starts reading cookies() to decide what a user may see, this
// file quietly stops being equivalent to its sibling.
import "server-only";
...
// THE HTTP BOUNDARY for /api/mcp — the counterpart of withRequestUser, and
// registered in scripts/guards.ts by name for the same reason: a route handler is
// only as authenticated as the named gate its body reaches.
//
// Three refusals before `fn` ever runs, in increasing cost order:
//
//   1. requireBearerAuth — no token, bad signature, no client_id, expired. Answers
//      with a fully-formed RFC 9728 challenge carrying resource_metadata, which is
//      how a cold client bootstraps. Returning our own tidier 401 here would break
//      discovery at step one.
//   2. mcpIdentity — the verified token carries no usable sub/email. Not reachable
//      through mcpTokenVerifier; this catches a future verifier that forgets.
...
// flipping it off has to refuse `initialize` and `tools/list` too, because an
// "off" server that still lets a client enumerate the tool surface and confirm the
// account exists is a strange sort of off. The price is one single-row transaction
// per request.
